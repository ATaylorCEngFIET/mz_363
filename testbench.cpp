...
//    std::string design_libname = getcurrentdir() + "/xsim.dir/adder/xsimk" + lib_extension;
    std::string design_libname = "xsim.dir/average/xsimk";
    ...
    char ip_val[8]  = {2, 3, 2, 3, 3, 3, 3, 2}; // X"0000"
    char sum_val[8]= { 2, 2, 2, 2, 3, 2, 3, 3}; // X"0000"

    // Temp values
    char temp_std_logic;
    char temp_std_logic_vector[8];
    ...
    try {
        ...
        ip = Xsi_Instance.get_port_number("ip");
        if(ip <0) {
          std::cerr << "ERROR: Port IP not found" << std::endl;
          exit(1);
        }
        op = Xsi_Instance.get_port_number("op");
        if(op <0) {
          std::cerr << "ERROR: Port Op not found" << std::endl;
          exit(1);
        }
		...
		Xsi_Instance.put_value(ip, ip_val);
		Xsi_Instance.run(10);
